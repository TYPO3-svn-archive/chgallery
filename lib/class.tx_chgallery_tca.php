  ...
  function user_chgallerydescr ($PA, $fobj) {
	...
	function user_chgallerypreview ($PA, $fobj) {
		...
		if (is_numeric($uid)) {
			...
			$flexformArray = t3lib_div::xml2array($PA['row']['pi_flexform']);
			...
			$languageid = trim($flexformArray['sys_language_uid']['vDEF']);
			...
			if ($path!='' && is_dir(PATH_site.$path)) {
				...
				$level = ($mode =='LIST') ? 1 : 0;
				$imageList = t3lib_div::getAllFilesAndFoldersInPath (array(), PATH_site.$path, $fileTypes, 0, $level);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/chgallery/class.tx_chgallery_tca.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/chgallery/class.tx_chgallery_tca.php']);
}
